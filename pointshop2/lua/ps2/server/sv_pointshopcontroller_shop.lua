...
--modif 09/01/24 (fix erreur déco)
LibK.GLib.PlayerMonitor:AddEventListener("PlayerDisconnected", "PS2_PlayerDisconnected", function(_, ply, userId)
    if not IsValid(ply) then
        KLogf(4, "Le joueur %s s'est déconnecté en étant invalide.")
        return
    end

    if not ply.PS2_Inventory or not ply.PS2_Inventory.getItems then
        KLogf(4, "Joueur %s : A quitté avant que l'inventaire n'ait été chargé", ply:Nick())

        if not ply.fullyLoadedPromise or not ply.fullyLoadedPromise.Reject then
            KLogf(4, "fullyLoadedPromise invalide ou manquante pour le joueur %s", ply:Nick())
        else
            ply.fullyLoadedPromise:Reject('Disconnected')
        end
    else
        for k, item in pairs(ply.PS2_Inventory:getItems()) do
            if KInventory.ITEMS and KInventory.ITEMS[item.id] then
    ...
    if not ply.PS2_Slots then
        ...
        for k, slot in pairs(ply.PS2_Slots) do
            if slot.Item then
                ...
                if KInventory.ITEMS and KInventory.ITEMS[slot.Item.id] then
